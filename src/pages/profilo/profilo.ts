...
export class ProfiloPage {
  ...
  updateProfile() {
    ...
    loader.present().then( () => {this.db.collection("/Utenti").doc(this.user_data.username).update({nome:this.user_data.name,cognome:this.user_data.surname,password:this.user_data.password,informazioni:this.user_data.description,email:this.user_data.email,nazionalita:this.user_data.nationality,sesso:this.user_data.gender});loader.dismissAll()}); // Get back to profile page. You should do that after you got data from API
  }
